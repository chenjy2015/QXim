...
import com.example.baselib.viewmodel.IModel
...
class PUtils {
    companion object {
        fun <T> createViewModel(`object`: Any): T? {
            ...
            try {
                for (t in pt.getActualTypeArguments()) {
                    if (IModel::class.java.isAssignableFrom(t as Class<*>)) {
                        modelClass = t
                        break
                    }
                }
        ...
        fun <T> createPresenter(`object`: Any): T? {
            val clz = `object`.javaClass
            val type = clz.genericSuperclass
            ...
            try {
                for (t in pt.getActualTypeArguments()) {
                    if (IPresenter::class.java.isAssignableFrom(t as Class<*>)) {
                        modelClass = t
                        break
                    }
                }
